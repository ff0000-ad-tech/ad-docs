...
const config = `${__dirname}/../md/config.jsdoc.json`

const hbsFiles = ['fromClasses', 'fromPackage'].map(name => {
	return new Promise((resolve, reject) => {
		fs.readFile(`${__dirname}/../md/${name}.hbs`, (err, data) => {
			if (err) reject(err)
			resolve(data)
		})
	})
...
Promise.all(hbsFiles).then(data => {
	const [fromClassesData, fromPackageData] = data
	const renderedOutputs = groupedInputs.map(obj => {
		...
		if (obj.files.length > 0) {
			...
			let dataChoice = fromClassesData
			// choose type
			switch (obj.name) {
				case 'ad-useragent':
					dataChoice = fromPackageData
					break
			}

			output = jsdoc2md.renderSync({
				data: templateData,
				template: dataChoice.toString(),
...
// fs.readFile(`${__dirname}/../md/README.hbs`, (err, data) => {
